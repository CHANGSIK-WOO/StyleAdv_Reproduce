...
def main(args):
    ...
    if utils.is_main_process():
        ...
        # Create acc file for recording results
        acc_file_path = output_dir / 'test_results.txt'
        with open(acc_file_path, 'w') as acc_file:
            print('Testing Results:', file=acc_file)
            print('Model:', args.output_dir, file=acc_file)
            print('n_shot:', args.nSupport, file=acc_file)
            ...
            for dataset in test_datasets:
                ...
                try:
                    test_bestmodel_bscdfsl_ViT(acc_file, args.output_dir.split('/')[-1], dataset, args.nSupport,
                                               save_epoch=-1)
