...
from .episodic_dataset_withGlobalID_20221102 import EpisodeDataset, EpisodeDataset_withGlobalID, EpisodeJSONDataset
...
def get_sets_withGlobalID(args):
    print(args.dataset == 'mini_imagenet')
    from .mini_imagenet import dataset_setting
    ...
    trainTransform, valTransform, inputW, inputH, \
        trainDir, valDir, testDir, nbCls = \
        dataset_setting(args.nSupport, args.img_size)

    trainSet = EpisodeDataset_withGlobalID(imgDir=trainDir,
                                           nCls=args.nClsEpisode,
                                           nSupport=args.nSupport,
                                           nQuery=args.nQuery,
                                           transform=trainTransform,
...
def get_loaders(args, num_tasks, global_rank):
    ...
    if args.eval:
        ...
        dataset_train, dataset_val, _ = get_sets(args)
...
def get_loaders_withGlobalID(args, num_tasks, global_rank):
    # datasets
    if args.eval:
        _, _, dataset_val = get_sets(args)
    else:
        # dataset_train, dataset_val, _ = get_sets(args)
        dataset_train, dataset_val, _ = get_sets_withGlobalID(args)
    worker_init_fn = None
