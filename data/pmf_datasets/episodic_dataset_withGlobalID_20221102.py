...
class EpisodeDataset_withGlobalID(data.Dataset):
    """
    Dataloader to sample a task/episode.
    In case of 5-way 1-shot: nSupport = 1, nCls = 5.
    ...
    def __init__(self, imgDir, nCls, nSupport, nQuery, transform, inputW, inputH, nEpisode=2000):
        ...
        self.GlobalID_S = intType(nCls * nSupport)
        self.GlobalID_Q = intType(nCls * nQuery)
    ...
    def __getitem__(self, idx):
        ...
        for i, cls in enumerate(clsEpisode) :
            ...
            for j in range(self.nSupport) :
                ...
                align_imgPath = imgPath
                
                # 直接在映射后的路径中查找完全匹配的路径
                try:
                    idx = self.mapped_meta_path.index(align_imgPath)
                    clsID = self.meta_clsID[idx]
                except ValueError:
                    # 如果找不到完全匹配的路径，报错并提供详细信息
                    raise FileNotFoundError(f"在base.json的映射路径中找不到图片: {align_imgPath}\n"
                                          f"请检查以下内容:\n"
                                          f"1. 图片文件是否存在于: {align_imgPath}\n"
                                          f"2. base.json中是否包含对应的路径映射\n"
                                          f"3. 路径映射是否正确")
                
                self.GlobalID_S[i * self.nSupport + j] = clsID
            ...
            for j in range(self.nQuery) :
                ...
                self.GlobalID_Q[i * self.nQuery + j] = clsID
        ...
        permSupport = torch.randperm(self.nCls * self.nSupport)
        permQuery = torch.randperm(self.nCls * self.nQuery)
        #print(self.GlobalID_S, self.GlobalID_Q)

        return (self.tensorSupport[permSupport],
               self.labelSupport[permSupport],
               self.tensorQuery[permQuery],
               self.labelQuery[permQuery],
               self.GlobalID_S[permSupport],
               self.GlobalID_Q[permQuery])
