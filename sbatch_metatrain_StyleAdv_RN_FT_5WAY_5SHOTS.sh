#!/usr/bin/env bash

#SBATCH --job-name=metatrain_StyleAdv_RN_5way_1shot
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
#SBATCH -p batch
#SBATCH -w vgi1
#SBATCH --cpus-per-gpu=8
#SBATCH --mem-per-gpu=20G
#SBATCH --time=1-0
#SBATCH -o ./logs/%N_%x_%j.out
#SBATCH -e ./logs/%N_%x_%j.err

set -euo pipefail

echo "Starting Sbatch metatrain_StyleAdv_RN_FT_5way_1shot"
#python3 finetune_StyleAdv_RN.py --testset cars --name exp-FT --train_aug --n_shot 5 --finetune_epoch 50 --resume_dir ./output/checkpoints/exp-name/RN_5WAY_5SHOTS --resume_epoch -1
python3 metatrain_StyleAdv_RN.py --dataset miniImagenet --name exp-name --train_aug --warmup baseline --n_shot 1 --stop_epoch 200
echo "Finish Sbatch metatrain_StyleAdv_RN_FT_5way_1shot"